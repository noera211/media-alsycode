                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <td class="px-5 py-3">
                        <span class="text-xs px-2.5 py-1 rounded-md font-medium
                            {{ $u->role === 'guru' ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-600' }}">
                            {{ ucfirst($u->role) }}
                        </span>
                    </td>
                    <td class="px-5 py-3 text-center">
                        <span class="text-xs px-2.5 py-1 rounded-full font-medium inline-flex items-center gap-1
                            {{ $u->is_active ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600' }}">
                            @if($u->is_active)
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            {{-- TOGGLE --}}
                            <form action="{{ route('admin.users.toggle', $u) }}" method="POST" class="inline-block" onsubmit="return confirm('Ubah status akun ini?')">
                                @csrf
                                <button
                            ...
                            <form
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 p-1.5 rounded-lg hover:bg-red-50 transition" title="Hapus Permanen">
...
{{-- ================= MODAL CREATE ================= --}}
<div id="modal-create" class="hidden fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm items-center justify-center px-4 transition-all">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="w-full border border-gray-300 rounded-xl pl-10 pr-10 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        <button type="button" onclick="togglePassword('create')" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                            <i data-lucide="eye-off" id="create-password-icon" class="w-4 h-4"></i>
                        </button>
                    </div>
                </div>
...
{{-- ================= MODAL EDIT ================= --}}
<div id="modal-edit" class="hidden fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm items-center justify-center px-4 transition-all">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div class="p-6">
            <div class="flex items-center gap-3 mb-5">
                <div class="p-2 bg-indigo-50 text-indigo-600 rounded-lg">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            class="w-full border border-gray-300 rounded-xl pl-10 pr-10 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        <button type="button" onclick="togglePassword('edit')" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                            <i data-lucide="eye-off" id="edit-password-icon" class="w-4 h-4"></i>
                        </button>
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-4">
...
{{-- ================= MODAL RESET ================= --}}
<div id="modal-reset" class="hidden fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm items-center justify-center px-4 transition-all">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            placeholder="Ketik password baru..."
                            class="w-full border border-gray-300 rounded-xl pl-10 pr-10 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        <button type="button" onclick="togglePassword('reset')" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                            <i data-lucide="eye-off" id="reset-password-icon" class="w-4 h-4"></i>
                        </button>
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-4">
...
<script>
    ...
    function openEditUser(id, name, email, role) {
        ...
        // Reset field password & icon setiap buka modal
        const input = document.getElementById('edit-password-input');
        input.value = '';
        input.type = 'text';
        document.getElementById('edit-password-icon').setAttribute('data-lucide', 'eye-off');
        lucide.createIcons();
    ...
    function openResetModal(id, name) {
        ...
        input.value = '';
        input.type = 'text';
        document.getElementById('reset-password-icon').setAttribute('data-lucide', 'eye-off');
        lucide.createIcons();
        openModal('modal-reset');
    }

    // Toggle show/hide password — prefix: 'create' | 'edit' | 'reset'
    function togglePassword(prefix) {
        const input = document.getElementById(prefix + '-password-input');
        const icon  = document.getElementById(prefix + '-password-icon');
        if (input.type === 'text') {
            input.type = 'password';
