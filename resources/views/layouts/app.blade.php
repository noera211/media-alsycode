...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="flex min-h-screen">
        ...
        {{-- Sidebar --}}
        <aside id="sidebar"
            class="w-64 bg-white border-r border-gray-200 flex flex-col fixed h-screen z-50 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out">
            ...
            <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                ...
                <button id="sidebar-collapse-btn"
                    class="hidden md:block p-1 rounded-md hover:bg-gray-100 transition-colors"
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                ...
                @else
                    ...
                    <a href="{{ route('pbl.index') }}"
                        class="sidebar-link {{ request()->routeIs('pbl*') ? 'active' : '' }}">
                        <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if (auth()->user()->isGuru())
                        <a href="{{ route('bank-soal.index') }}"
                            class="sidebar-link {{ request()->routeIs('bank-soal*') ? 'active' : '' }}">
                            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('compiler') }}"
                        class="sidebar-link {{ request()->routeIs('compiler*') ? 'active' : '' }}">
                        <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="border-t border-gray-100" id="user-info-wrap">
                <div id="user-info-expanded" class="p-4">
                    <div class="flex items-center gap-3 mb-3">
                        <div
                            class="h-9 w-9 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-sm shrink-0">
                ...
                <div id="user-info-collapsed" class="hidden flex-col items-center py-4 gap-3">
                    <div
                        class="h-9 w-9 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-sm">
                        {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                    </div>
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
        ...
        <main class="flex-1 ml-0 md:ml-64 transition-all duration-300 ease-in-out min-w-0" id="main-content">
            ...
            {{-- Mobile Navbar --}}
            <nav
                class="bg-white border-b border-gray-200 px-4 py-3 md:hidden flex items-center justify-between sticky top-0 z-20">
                <button class="p-2 rounded-md hover:bg-gray-100 transition-colors" onclick="toggleSidebar()">
                ...
                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/logo (2).png') }}" alt="ALSYCODE" class="h-6 w-auto">
    ...
    <script>
        ...
        function toggleCollapse() {
            ...
            const expanded = document.getElementById('user-info-expanded');
            const collapsed = document.getElementById('user-info-collapsed');

            sidebarCollapsed = !sidebarCollapsed;

            if (sidebarCollapsed) {
                // Hapus tulisan md: pada sidebar agar cocok dengan class di HTML
                ...
                links.forEach(l => l.classList.add('collapsed'));
                ...
                links.forEach(l => l.classList.remove('collapsed'));
