        ...
        <div class="hero-banner">
            ...
            <div class="relative z-10 hidden sm:block">
                <svg width="120" height="100" viewBox="0 0 120 100" fill="none">
                    ...
                    <rect x="20" y="20" width="80" height="55" rx="6" fill="rgba(255,255,255,0.15)"
                    ...
                    <circle cx="92" cy="18" r="10" fill="rgba(255,255,255,0.2)" stroke="rgba(255,255,255,0.4)"
                        stroke-width="1.5" />
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div class="lg:col-span-2 bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <a href="{{ route('pbl.index') }}"
                        class="text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">Lihat Semua
                        →</a>
                </div>
                    ...
                    @forelse($recentSubmissions as $sub)
                        ...
                        <div class="submission-row px-5 py-3.5 flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="avatar-circle {{ $color }}">
                                    {{ strtoupper(substr($sub->student->name ?? '?', 0, 2)) }}</div>
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-gray-800 truncate">{{ $sub->student->name }}</p>
                                    <p class="text-xs text-gray-400 truncate">{{ $sub->activity->title }} ·
                                        {{ $sub->submitted_at->diffForHumans() }}</p>
                                </div>
                            </div>
                            @if ($sub->nilai !== null)
                                <span
                                    class="badge-pill shrink-0 {{ $sub->nilai >= 80 ? 'bg-emerald-100 text-emerald-700' : ($sub->nilai >= 60 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700') }}">{{ $sub->nilai }}</span>
                            @else
                                <span class="badge-pill shrink-0 bg-amber-50 text-amber-600 border border-amber-200">Belum
                                    dinilai</span>
                            @endif
                        </div>
                    @empty
                        <div class="px-5 py-12 text-center">
                            <i class="icon-inbox"
                                style="font-size:2rem;color:#d1d5db;display:block;margin:0 auto 0.75rem;"></i>
                ...
                <div class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                    ...
                    <div class="space-y-2.5">
                        <a href="{{ route('materi.index') }}" class="quick-action">
                            <div class="quick-action-icon bg-indigo-100 text-indigo-600"><i class="icon-book-open"></i>
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-gray-800">Kelola Materi</p>
                                <p class="text-xs text-gray-400">Tambah & edit konten</p>
                            </div>
                        </a>
                        <a href="{{ route('pbl.index') }}" class="quick-action">
                            <div class="quick-action-icon bg-pink-100 text-pink-600"><i class="icon-layers"></i></div>
                            <div>
                                <p class="text-sm font-semibold text-gray-800">Aktivitas PBL</p>
                                <p class="text-xs text-gray-400">Nilai submission siswa</p>
                            </div>
                        </a>
                        <a href="{{ route('nilai.index') }}" class="quick-action">
                            <div class="quick-action-icon bg-amber-100 text-amber-600"><i class="icon-chart-bar"></i>
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-gray-800">Rekap Nilai</p>
                                <p class="text-xs text-gray-400">Lihat nilai akhir siswa</p>
                            </div>
                        </a>
                        <a href="{{ route('bank-soal.index') }}" class="quick-action">
                            <div class="quick-action-icon bg-violet-100 text-violet-600"><i
                                    class="icon-clipboard-list"></i></div>
                            <div>
                                <p class="text-sm font-semibold text-gray-800">Bank Soal</p>
                        ...
                        <a href="{{ asset('files/guidebook.pdf') }}" target="_blank" class="quick-action">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-800">Guide Book</p>
                                <p class="text-xs text-gray-400">Panduan belajar & referensi materi</p>
                            </div>
                        </a>
                    </div>
                </div>
